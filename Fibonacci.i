...
void PrintFibonacci(struct Fibonacci* fibonacciPtr, unsigned long long int ordinal) {
 ...
 for(int i = (99999999 - fibonacciPtr -> size); i < 99999999; i++) {
...
long double CalculateExecutionTime(clock_t startTime, clock_t endTime) {
 return (long double)(endTime - startTime)/(
# 36 "Fibonacci.c" 3 4
                                           ((__clock_t) 1000000)
# 36 "Fibonacci.c"
                                                         );
...
# 150 "Fibonacci.c"
 ...
 for(; j < (range - 1); j++) {
  ...
  fibonacciNumber1Ptr = &fibonacci[(j + 0) % 3];
  fibonacciNumber2Ptr = &fibonacci[(j + 1) % 3];
  fibonacciSumPtr = &fibonacci[(j + 2) % 3];
  ...
  if(isInitialRun) {
   ...
   fibonacciNumber1Ptr -> number[99999999 - 1] = 0;
   fibonacciNumber2Ptr -> number[99999999 - 1] = 1;
...
# 173 "Fibonacci.c"
  ...
  for(i = (99999999 - 1); i > (99999999 - fibonacciSumPtr -> size - 1); i--) {
   ...
   if (sum > 9) {
    carry = 1;
    if((99999999 - fibonacciNumber2Ptr -> size) == i) {
